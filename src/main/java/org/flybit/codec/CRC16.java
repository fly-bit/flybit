...
public class CRC16{
    ...
    private static final int[] CRC16_TABLE = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108,
    ...
    public static int crc16(final byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++)
            crc = (crc << 8) ^ CRC16_TABLE[((crc >>> 8) ^ bytes[i]) & 0x00FF];
        return crc & 0xFFFF;
    }
    ...
    private int updateBytes(int crc, byte[] b, int off, int len){
        for (int i = 0; i < len; i++)
            crc = (crc << 8) ^ CRC16_TABLE[((crc >>> 8) ^ b[off+i]) & 0x00FF];
        return crc;
    }
